...
class CalculTime extends MonoBehaviour
{
...
//______________________________________________________________________________________________________________________________________________________________
	...
	public  var PlayerClient    : boolean    ; // est-ce que le joueur est client ? 
	public 	var finPartieServer : GameObject ; // le gameObject côté serveur qui enregistre le temps en local
...
//______________________________________________________________________________________________________________________________________________________________
	...
	function Start()
	{
		...
		if(Network.isServer) // si on est serveur. 
		{
			this.PlayerClient = false; // on ne controle pas le temps
			this.finPartieServer = GameObject.Find("finPartie");
		}
		else // sinon si on est client ( donc le 2em joueur est entré dans la partie )
		{
			this.PlayerClient = true ; // on controle le déroulement du temps et l'envoi en réseau au joueur serveur.
		}
	}
	...
	{
		if (this.PlayerClient) // si on est le client
		{
			calculScore(); // on calcul le temps
		}
		else
		{
			this.finPartieServer.GetComponent(FinPartieControl).score = this.getConstantTime();
		}
...
//_______________________________________________________________________________________________________________________________________________________
	...
	public function calculScore()
	{
		...
		setConstantTime(constantTime+Time.fixedDeltaTime);
		if(getConstantTime()>=getTimeScore() + 1) //si une segonde est passée
		{
			/* on envoi en réseau le changement de valeur du temps */
			networkView.RPC("calculScoreRPC", RPCMode.AllBuffered);
		}
	}
	@RPC
	function calculScoreRPC() // la partie envoyé en réseau
	{
